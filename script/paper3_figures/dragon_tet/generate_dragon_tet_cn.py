...
def configure_chinese_font():
    """Select a local CJK font and configure PDF-safe embedding."""
    preferred = [
        "Noto Sans CJK SC",
        "Source Han Sans CN",
        "WenQuanYi Micro Hei",
    ...
    chosen = next((name for name in preferred if name in available), preferred[0])
...
def extract_log_data(log_dir):
    ...
    for folder in sorted(glob.glob(pattern)):
        ...
        # Parse NEP and np from folder name: NEP_40_PD_np4
        ...
        # Sum all CMS::solve time occurrences
...
def plot_breakdown(df):
    ...
    # Colors from unified breakdown palette
    COMPONENT_MAP = {
    ...
    # # Secondary x-axis labels for np
    # for i, nep in enumerate(nep_vals):
    #     base = i * (n_np * bar_width + group_spacing)
    #     for j, np_val in enumerate(np_vals):
    #         x_pos = base + j * bar_width
    #         ax.text(
    #             x_pos,
    #             -max(bottoms) * 0.06,
    #             f"np{np_val}",
