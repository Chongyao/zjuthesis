\subsubsection{预条件器正定性证明}\label{app:invertible}

\textbf{正定性证明：} 不失一般性，我们假设最大坐标的系统矩阵 $\mathbf{H}$ 是对称正定的。（否则，我们可以添加 $\alpha \I$ 来确保正定性。）那么其子块 $\mathbf{H}_{\mathbf{p}}$ 和 $\mathbf{H}_{\mathbf{q}}$ 也是对称正定的。

我们的预条件器基于公式~\ref{eq:preconditioner_structure} 具有以下块结构：
\begin{equation}
  \begin{aligned}
...
根据公式~\ref{eq:A}，我们指出 $\J_{\mathbf{p, s}}^{\top}\mathbf{H}_{\mathbf{p}}\J_{\mathbf{p, s}}$ 和 $\J_{\mathbf{q, s}}^{\top}\mathbf{H}_{\mathbf{q}}\J_{\mathbf{q, s}}$ 至少都是半正定的，因为：
\begin{equation}
  \forall \mathbf{v} \ne \mathbf{0}, \quad
...
接下来，我们证明 $\J_{\mathbf{q,\omega}}^{\top}\mathbf{H}_{\mathbf{q}}\J_{\mathbf{q,\omega}}$ 是正定的。

从 $\omega$ 到 $\mathbf{q}$ 的映射 $\mathcal{T}_{\mathbf{q,\omega}}$ 是局部单射映射，因此其雅可比 $\J_{\mathbf{q,\omega}}$ 具有满列秩。考虑到 $\mathbf{H_{\mathbf q}}$ 的正定性，我们可以将其分解为 $\mathbf{LL}^{\top}$，并且：
\begin{equation}
  \begin{aligned}
