...
%   \caption{\textbf{CB-CMS 模态示例。} \textbf{左图：} 由 \Cref{eq:sub-eig} 求解的第 1 个局部特征模态。\textbf{右图：} 由 \Cref{eq:traditional-response} 求解的界面模态之一。}
%   \label{eq:example-cms-modes}
% \end{figure}
...
\begin{figure}
  ...
  %%\includegraphics[width=0.49\linewidth]{substructure_analysis/schur_vs_eig_timing_curves.png}
  %% \includegraphics[width=0.49\linewidth]{substructure_analysis/schur_cost_breakdown.png}
  \caption{\textbf{传统 CMS 并行部分计算代价分析。} 一个方形网格由 size $\times$ size 个顶点离散化。\textbf{左图：} $t_{\text{sub-eig}}$ 与特征模态数量呈超线性关系。当子结构特征模态数量较少时，固定代价 $t_{\text{sub-schur}}$（红色虚线）接近 $t_{\text{sub-eig}}$。
    %% \textbf{右图：} Schur 补代价分解（$t_{\text{sub-schur}}$）。
    %% 代价主要来自重复求解和矩阵乘法，而非初始分解。
  }
...
\Cref{fig:breakdown-CMS}（左）比较了 $t_{\text{sub-eig}}$（实线）与 $t_{\text{sub-schur}}$（虚线）。%% These combined bottlenecks—the high fixed cost of interface
%% computation and the poor scaling of adding high-frequency interior modes
%% severely hinder traditional CMS for super-large-scale problems and motivate our
%% alternative approach. .... This makes the Schur complement a significant
%% computational burden.

%% \Cref{fig:breakdown-CMS} (right) breaks down
%% $t_{\text{sub-schur}}$. The factorization cost is negligible; the
%% dominant expense stems from the numerous forward/backward
%% substitutions and matrix multiplications, both scaling with the
%% potentially large number of interface DOFs, $n_{b,i}$. , particularly for
%% complex geometries.
