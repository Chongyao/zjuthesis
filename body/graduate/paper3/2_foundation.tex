...
\begin{figure}[H]
  ...
  \caption{\textbf{传统模态综合法并行部分的计算代价分析。} 在一个正方形网格上，以 size$\times$size 个顶点离散。左图中，$t_{\text{sub-eig}}$ 随特征模态数量呈超线性增长；当子结构特征模态数量较少时，固定代价 $t_{\text{sub-schur}}$（红色虚线）与 $t_{\text{sub-eig}}$ 接近。}
